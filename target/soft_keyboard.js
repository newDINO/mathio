...
var ShiftState;
(function (ShiftState) {
    ShiftState[ShiftState["Short"] = 0] = "Short";
    ShiftState[ShiftState["Long"] = 1] = "Long";
    ShiftState[ShiftState["Not"] = 2] = "Not";
...
class FirstPage {
    ...
    on_key(key) {
        if (this.on_key_handle)
            this.on_key_handle(key);
...
function set_scale(e, scale) {
    e.style.width = e.style.height = scale;
}
class CharKey {
    constructor(key, on_key) {
        this.key = key;
        this.element = html_e('div');
...
function same_size_inner() {
    let result = html_e('div');
    set_scale(result, '100%');
    result.style.position = 'absolute';
    return result;
}
function char_key(c, on_key) {
    let result = html_e('div');
    result.style.width = '10%';
    let inner = create_inner();
    result.appendChild(inner);
...
function common_key(key, s_key, on_key) {
    let result = html_e('div');
    result.style.width = '10%';
    let inner = create_inner();
    result.addEventListener('pointerdown', () => {
...
function common_key_s(key, s_key, on_key) {
    let result = html_e('div');
    result.style.width = '10%';
    let inner = create_inner();
    result.addEventListener('pointerdown', () => {
